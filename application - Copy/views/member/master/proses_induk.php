   ...
   <div class="inner-wrapper">
    ...
    <section role="main" class="content-body">
     <header class="page-header">
      <h2>Master proses_induk <?php echo $id_proses_induk; ?></h2>
    </header>
    <!-- start: page -->
    <section class="panel">
      <header class="panel-heading">
        <div class="row show-grid">
          <div class="col-md-6" align="left"><h2 class="panel-title">Data  proses_induk</h2></div>
          <?php
          echo level_user('master','proses_induk',$this->session->userdata('kategori'),'add') > 0 ? '<div class="col-md-6" align="right"><a class="btn btn-success" href="#"  data-toggle="modal" data-target="#tambahData"><i class="fa fa-plus"></i> Tambah</a></div>':'';
          ?>
        </div>
      </header>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <section class="panel panel-primary">
        <?php echo form_open('master/proses_induktambah',' id="FormulirTambah"');?>
        <header class="panel-heading">  
          <h2 class="panel-title">Tambah proses_induk</h2>
        </header>
        <div class="panel-body">
          <div class="form-group mt-lg">
            <label class="col-sm-3 control-label">Proses Induk<span class="required">*</span></label>
            <div class="col-sm-9">
              <input type="hidden" name="id_proses_induk" id="id_proses_induk" value="<?php echo $id_proses_induk ?>">

              <input type="text" class="form-control" readonly value="<?php echo $id_proses_induk ?>" />
            </div>
          </div>
          <div class="form-group mt-lg">
            <label class="col-sm-3 control-label">Tanggal<span class="required">*</span></label>
            <div class="col-sm-9">
              <input type="text" name="tgl_proses_induk" class="form-control tanggal_masa" data-plugin-datepicker required/>
            </div>
          </div>
...
<div class="modal fade" data-keyboard="false" data-backdrop="static"  id="detailData" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <section class="panel  panel-primary">
        <header class="panel-heading">
          <h2 class="panel-title">Detail proses_induk</h2>
        </header>
...
<div class="modal fade" data-keyboard="false" data-backdrop="static"  id="editData" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <section class="panel  panel-primary">
        <?php echo form_open('master/proses_indukedit',' id="FormulirEdit"');?>
        <input type="hidden" name="idd" id="idd">
        <header class="panel-heading">
          <h2 class="panel-title">Edit Data proses_induk</h2>
        </header>
        <div class="panel-body">
          <div class="form-group mt-lg">
            <label class="col-sm-3 control-label">Tanggal<span class="required">*</span></label>
            <div class="col-sm-9">
              <input type="text" name="tgl_proses_induk" id="tgl_proses_induk" class="form-control tanggal_masa" data-plugin-datepicker required/>
            </div>
          </div>
    ...
    <div class="modal-content">
      <section class="panel  panel-danger">
        ...
        <footer class="panel-footer">
          <div class="row">
            <div class="col-md-12 text-right">
              <?php echo form_open('master/proses_indukhapus',' id="FormulirHapus"');?>
...
<script type="text/javascript">
  ...
  var tablepembeli = $('#pembelidata').DataTable({
    ...
    "order": [],
    "ajax": {
      "url": "<?php echo base_url()?>master/dataproses_induk",
      "type": "GET"
    },
    "columnDefs": [
    {
  ...
  function detail(elem){
    ...
    $.ajax({
      type: 'GET',
      url: '<?php echo base_url()?>master/proses_indukdetail',
      data: 'id=' + dataId,
      dataType 	: 'json',
      success: function(response) {
        ...
        $.each(response, function(i, item) {
          ...
          datarow+="<tr><td>Tanggal</td><td>: "+item.tgl_proses_induk_indo+"</td></tr>";
          datarow+="<tr><td>Proses Induk</td><td>: "+item.id_proses_induk+"</td></tr>";
  ...
  function edit(elem){
    ...
    $.ajax({
      ...
      success: function(response) {
        $.each(response, function(i, item) {
          document.getElementById("tgl_proses_induk").setAttribute('value', item.tgl_proses_induk);
