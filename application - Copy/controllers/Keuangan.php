...
{   
    ...
    foreach($query->result() as $r) {  
        if ($r->status_bayar=='0') {
            $status = '<a href="'.base_url().'Keuangan/update_status/'.$r->id_pembayaran.'/'.$kode_item.'" class="btn btn-danger">Belum Terbayar</a>';
            // $status ='lunas';
        }else{
            $status = '<a href="'.base_url().'Keuangan/update_status/'.$r->id_pembayaran.'/'.$kode_item.'" class="btn btn-success">Sudah Terbayar</a>';
        }
        $data[] = array(   
            ' 
            <div class="btn-group dropup">
