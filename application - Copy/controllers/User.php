...
if ($this->input->post('id_penjual')!='') {
    redirect('master/penjual','refresh');
}else{
    echo json_encode($data); 
}
}
...
public function userdetail(){  
    cekajax(); 
    $idd = $this->input->get("id");   
    $query = $this->db->select("a.id, a.kategori, a.username, a.nama_admin, a.email, a.aktif, b.kategori_user")->from("master_admin a")->join('kategori_user b', 'a.kategori = b.id')->where('a.id', $idd, 1)->get(); 
