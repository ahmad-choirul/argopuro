...
class Pembelian_model extends CI_Model{  
     ...
     var $column_search_pilihanitem = array('kode_item','nama_item','stok'); 
     var $column_order_pilihanitem = array('kode_item','nama_item','stok',null);
    ...
    function simpandatapembelian(){   
        ...
        for($i = 0; $i < count($kode_item); $i++){         
            ...
            $total_harga[$i] =  ($kuantiti[$i]*$harga[$i])-(($kuantiti[$i]*$harga[$i])*($diskon[$i]/100));    
            $listitem = array(
                'nomor_faktur'=>$nomor_faktur,  
                'kode_item'=>$kode_item[$i],  
                'sku'=>$sku[$i],  
            ...
            // 
            $dt = $this->db->get_where("master_item", array('kode_item', $kode_item[$i])); 
            // cek expired beda atau sama
            if($tgl_expired[$i] != $dt->row->tgl_expired ){
                $listharga = array(
                    ...
                    'harga_jual_4'=>$harga4[$i], 
                    'stok'=> $dt->row->stok + $kuantiti[$i],
                );
                $this->db->insert("master_item", $listharga);
            }else{
                $listharga = array(
                    'harga_jual'=>$harga1[$i],  
                    'harga_jual_distributor'=>$harga2[$i],  
                ...
                $this->db->update("master_item", $listharga); 
            }
    ...
    function recvalid($nomor_rec){   
        return  $this->db->get_where(
            'penerimaan_barang',array(
                'nomor_rec' => $nomor_rec, 
            )
        ); 
    }  
    ...
    function simpandatapenerimaan(){   
        ...
        for($i = 0; $i < count($kode_item); $i++){         
            ...
            $hitung=$oldkuantiti[$i]-$kuantiti[$i];
            $datapo= array('kuantiti'=> $hitung,);
            $this->db->where('kode_item', $kode_item[$i]);
            $this->db->update("purchase_order_detail", $datapo);
