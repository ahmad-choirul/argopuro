...
class Stok_model extends CI_Model
{
    ...
    private function _get_query_datakartustok()
    {
        $get = $this->input->get();
        if ($get['kode_item']!='') {
            $this->db->where('kode_item', $get['kode_item']);
        }
    ...
    function get_datakartustok_datatable()
    {
     ...
     if ($kode_item!='') {
        $this->db->where('kartu_stok.kode_item', $kode_item);
    }
    $jns_select = $this->input->get('jns_select');
    $search_jns_select = $this->input->get('search_jns_select');
    if ($jns_select!='') {
        if ($jns_select=='rec_penerimaan')
        {
         $this->db->where('nomor_rec_penerimaan', $search_jns_select);
     }       
     if ($jns_select=='id_utility') {
         $this->db->where('id_utility', $search_jns_select);
     }
     if ($jns_select=='id_stok_opname') {
         $this->db->where('id_stok_opname', $search_jns_select);
     }
     if ($jns_select=='id_stok_keluar') {
         $this->db->where('id_stok_keluar', $search_jns_select);
     }
     if ($jns_select=='id_penjualan') {
         $this->db->where('id_penjualan', $search_jns_select);
     }
 }
 $this->db->select('*');
...
public function listitem()
{
    ...
    foreach ($hasil as $key => $value) {
        $listitem[$value->kode_item]=$value->nama_item;
    }
    return $listitem;
}
...
public function listkasir()
{
    ...
    $this->db->where('kategori', '32');
    $hasil =  $this->db->get()->result();
    foreach ($hasil as $key => $value) {
        $listitem[$value->id]=$value->username;
    }
    return $listitem;
}
...
var $column_order_stokutility = array(null, 'nomor_ref', 'ket_utility', 'stok_sebelum', 'aksi', 'jumlah', 'waktu');
var $order_stokutility = array('waktu' => 'DESC');
private function _get_query_stokutility()
{
  $kode_item = $this->input->get('kode_item');
  if ($kode_item!='') {
    $this->db->where('u.kode_item', $kode_item);
}
...
if($firstdate!='' AND $lastdate!=''){
    $this->db->where('u.waktu BETWEEN "'.$firstdate. '" and "'. $lastdate.'"');
}
$get = $this->input->get();
