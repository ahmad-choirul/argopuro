...
class Pembelian extends CI_Controller {   
    ...
    public function pembeliantambah(){ 
        ...
        $povalid=$simpan->recvalid($nomor_po);  
		if ($this->form_validation->run() == FALSE){
			$errors = $this->form_validation->error_array();
			$data['errors'] = $errors;
        }else{            
            if($kategori == "Purchase Order" AND $povalid->num_rows() < 1){ 
                $errors['jumlah_obat'] = "Nomor Record tidak valid";
                $data['errors'] = $errors;
            } else {
                if(isset($kode_item) === TRUE AND $kode_item[0]!='')
                {  
    ...
    public function pembeliandetail(){  
        ...
        foreach($detailpo->result() as $r) {   
            ...
            $subArray['harga1']=$this->security->xss_clean(rupiah($r->harga_jual));
            ...
            $subArray['harga4_int']=$this->security->xss_clean(bilanganbulat($r->harga_jual_4));  
