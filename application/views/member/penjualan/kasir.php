								...
								<div class="row">
									<div class="col-md-6">
										<div class="form-group">
											<label class="control-label">Nama penjual / penjual</label>
											<!-- <select class="form-control cara_bayar" name="nama_penjual" id="nama_penjual">
												<?php foreach ($penjual as $s): ?>
													<option value="<?= $s['id']; ?>"><?= $s['nama_penjual'] ?></option>
												<?php endforeach ?>  
											</select>  -->
											<h4><?php echo $this->session->userdata('nama_admin'); ?></h4>
											<input type="text" hidden="" name="nama_penjual" value="<?php echo $this->session->userdata('idadmin'); ?>">
										</div>
									</div>
									<div class="col-md-6">
										<div class="form-group"> 
		...
		function keranjang(){   
			...
			$.ajax({
				...
				success: function(response) { 
					...
					if(response.datasub !=''){          
						...
						datarow+='<thead>';
						datarow+='<tr><th>Produk</th><th>Kuantiti</th><th style="min-width:70px;">Harga</th><th style="min-width:90px;">Total</th><th></th></tr></thead><tbody>';            
						$.each(response.datasub, function(i, itemsub) {                         	
							datarow+='<tr>';
            ...
            $(document).on('click','#modal-payment #paymenttransaksi',function(e) { // tombol paymenttransaksi sg ono ng modal payment
            	...
            	if (totalbayar>=totalbelanja) {
            		$('#modal-payment form').attr('action','<?=base_url()?>penjualan/struk?t='+idk+'&idd='+idkd+'&bayar='+totalbayar)
            		.submit()
            	}else{
            ...
            $(document).on('click','#modal-payment #paymenttransaksikredit',function(e) { // tombol paymenttransaksikredit sg ono ng modal payment
            	...
            	if (totalbayar>=totalbelanja) {
            		...
            		if (tanggal_jatuh_tempo!=null) {
            			...
            			$('#modal-payment form').attr('action','<?=base_url()?>penjualan/struk_kredit?t='+idk+'&penjual='+penjual+'&tmp='+tmp+'&idd='+idkd+'&bayar='+totalbayar)
            			.submit()
            		}else{
            			window.setTimeout(function() {  
            				new PNotify({
